...
This repository builds a static Astro site for `https://codeworkslabs.dev/`. Wrangler reads `wrangler.jsonc` and targets the Cloudflare Worker configuration named `codeworkslabs-dev`.
...
4. Run `npm run build`.
5. Run `npm run verify:image-rights`.
...
`CodeWorksLabs/codeworkslabs.dev`. A push to `main` runs `npm run build` and
then `npx wrangler deploy` from the repository root. Non-production branch
